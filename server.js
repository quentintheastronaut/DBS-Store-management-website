...
function fetchData(response){
    executeQuery('SELECT * FROM dbo.CHI_NHANH ORDER BY TEN_CHI_NHANH',function(result){
        ...
        response.write('<a href="http://localhost:5000/showroomlist/"><input id="search" type="submit" value="Submit"></a>')
        response.write('</form>')
...
app.post('/showroomlist/add', function(request, response){
    ...
    if(back == 1){
        ...
        alert('Store\'s id must have 8 characters');
        response.redirect('/showroomlist/add');
    }
    else if (manager.length != 6){
        alert('Manager\'s id must have 6 characters');
        response.redirect('/showroomlist/add');
    }
    else if (tele.length != 10 ){
        alert('Manager\'s id must have 10 numbers');
        response.redirect('/showroomlist/add');
    } 
    else {
...
app.get('/showroomlist/search', function (req, response) { 
...
app.get('/check',function(req, res){
    ...
    executeQuery('SELECT * FROM dbo.CHI_NHANH',function(result){
        console.log('here')
        res.send(result)
    })
})